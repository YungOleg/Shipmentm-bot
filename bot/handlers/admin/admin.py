...
async def wait_username_for_delete(call: CallbackQuery, state: FSMContext):
...
async def admin_delete_order_by_username(message: Message, state: FSMContext, session_maker: AsyncSession):
